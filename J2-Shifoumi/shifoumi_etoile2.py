...
equivalent = {'A':0,'B':1,'C':2,'X':0,'Y':1,'Z':2}
gagne =[['Z','X','Y'],['X','Y','Z'],['Y','Z','X']]
def coupAJouer(j1, gagnant):
    coupAJouer = gagne[equivalent[j1]][equivalent[gagnant]]
    return coupAJouer

#Remplacage des coups joues corespondants
for i in range(len(tours)):
    tours[i][1] = coupAJouer(tours[i][0], tours[i][1])
